	...
	for (var propertyId in blayers) {
		...
		if (Galv.LG.isEmpty(blayers[propertyId]) || blayers[propertyId]["graphic"] == "") {
			...
			//this._battleField.addChildAt(this.layerGraphics[propertyId],z);
                       SceneManager._scene.addChild(this.layerGraphics[id]); // 强行加到场景最顶层
		};
	};
};
...
// === 终极置顶补丁 ===
// 目标：强制把 Galv 图层插入到整个游戏画面的最顶层，盖住包括“图片”在内的一切

// 保存原本的创建方法
var _Galv_LG_SpritesetMap_createLayerGraphics = Spriteset_Map.prototype.createLayerGraphics;
Spriteset_Map.prototype.createLayerGraphics = function() {
    // 先执行插件原本的创建逻辑
    _Galv_LG_SpritesetMap_createLayerGraphics.call(this);
    
    // 关键逻辑：如果已经获取到了场景对象，并且图层容器存在
    if (SceneManager._scene && this._layerGraphics) {
        // 1. 先确保它不在其他地方挂着（防止重复添加报错）
        if (this._layerGraphics.parent) {
            this._layerGraphics.parent.removeChild(this._layerGraphics);
        }
        
        // 2. 直接添加到场景的最顶层容器
        // 这里我们尝试直接加到 _spriteset 上，这是最稳妥的位置
        var spriteset = SceneManager._scene._spriteset;
        if (spriteset) {
            // 强制把 Galv 图层加在 spriteset 的最顶层
            spriteset.addChild(this._layerGraphics);
            
            // 再次强制排序：把它放到所有子对象的最后（也就是最上面）
            spriteset.setChildIndex(this._layerGraphics, spriteset.children.length - 1);
        }
    }
};
})();
